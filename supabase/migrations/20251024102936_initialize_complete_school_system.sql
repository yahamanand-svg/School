/*
  # Complete School Management System with Teacher Class-Section Mapping

  ## Overview
  This migration creates the complete school management system with proper class/section structure
  and adds a dedicated table for mapping teachers to specific class-section combinations.
  ...
  - `teacher_class_sections`: NEW - Maps teachers to specific class-section-subject combinations
  ...
  ## Teacher Class-Section Assignment System
  
  The new `teacher_class_sections` table allows precise mapping:
  - A teacher assigned to Class 9 sections A and B will have separate entries:
    - { class: "9", section: "A", subject: "Maths" }
    - { class: "9", section: "B", subject: "Maths" }
...
DROP FUNCTION IF EXISTS get_teacher_class_sections(uuid) CASCADE;
DROP FUNCTION IF EXISTS get_teacher_formatted_classes(uuid) CASCADE;
...
-- NEW: Teacher Class-Section Mapping Table
...
CREATE INDEX idx_teacher_class_sections_class ON teacher_class_sections(class_name, section);
CREATE INDEX idx_teacher_class_sections_composite ON teacher_class_sections(teacher_id, class_name, section);
...
-- NEW: Function to get all class-section combinations for a teacher
...
-- NEW: Function to get formatted class-section strings (e.g., ["9A", "9B", "10A"])
CREATE OR REPLACE FUNCTION get_teacher_formatted_classes(p_teacher_id uuid)
RETURNS text[] AS $$
BEGIN
  RETURN ARRAY(
...
CREATE POLICY "Anyone can read teacher class sections" ON teacher_class_sections FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY "Anyone can insert teacher class sections" ON teacher_class_sections FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY "Anyone can update teacher class sections" ON teacher_class_sections FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true);
CREATE POLICY "Anyone can delete teacher class sections" ON teacher_class_sections FOR DELETE TO anon, authenticated USING (true);
